...
async function init(){
  ...
  let profile=fallbackProfile, rombel=[], eskul=[], activities=[], news=[], announcements=[], achievements=[], gallery=[], documents=[], schedules=[];
  if(SDN11.configured){
    const [p,r,e,a,n,an,ac,g,d,s]=await Promise.all([
    ...
    if(p.data)profile={...profile,...p.data};rombel=r;eskul=e;activities=a;news=n;announcements=an;achievements=ac;gallery=g;documents=d;schedules=s;
  }
  renderProfile(profile,rombel,eskul);renderRombel(rombel);renderEskul(eskul);renderActivities(activities);renderNews(news);renderAnnouncements(announcements);renderAchievements(achievements);renderGallery(gallery);renderDocuments(documents);renderSchedules(schedules);
  const toggle=document.querySelector(".nav-toggle"),nav=$("mainNav");toggle?.addEventListener("click",()=>nav.style.display=nav.style.display==="flex"?"none":"flex");
}
function renderProfile(p,rombel,eskul){
  const name=p.name||"SDN Larangan 11";$("brandName").textContent=name;$("brandCity").textContent=p.city||"Kota Tangerang";$("heroSchool").textContent=name;$("heroCardName").textContent=name;$("heroNpsn").textContent=p.npsn||"-";$("heroAddress").textContent=p.address||"-";$("heroSubtitle").textContent=p.hero_subtitle||p.vision||"";$("statStudents").textContent=p.students??"—";$("statRombel").textContent=rombel.length||"—";$("statEskul").textContent=eskul.length||"—";$("statAccreditation").textContent=p.accreditation||"—";$("profileTitle").textContent=p.profile_title||"Rumah Belajar yang Aman, Aktif, dan Berkarakter";$("profileDescription").textContent=p.description||"";$("visionText").textContent=p.vision||"";
